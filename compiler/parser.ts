...
module TypeScript {
    ...
    export class Parser {
        ...
        //TODO:add code for parseComment
	    ...
	    var endingToken = new ASTSpan();
            ...
            this.chkCurTok(TokenID.END, "Expected 'end'", errorRecoverySet);
        ...
        public isValidImportPath(importPath: string) {
            ...
            public parseModuleDecl(errorRecoverySet: ErrorRecoverySet, modifiers: Modifiers): ModuleDecl {
               ...
               // this.chkCurTok(TokenID.LCurly, "Expected '{'", errorRecoverySet | ErrorRecoverySet.ID);
                this.parseStmtList(errorRecoverySet | ErrorRecoverySet.End, moduleBody, true, BlockEndElements.None,
            ...
            public parseTypeReference(errorRecoverySet: ErrorRecoverySet, allowVoid: bool): AST {
                ...
                switch (this.tok.tokenId) {
                    ...
                    //case TokenID.ELSEIF://TODO:else if error
                    //    this.tok = this.scanner.scan();
            ...
            public parseFunctionStatements(   errorRecoverySet: ErrorRecoverySet,
                ...
                if (!requiresSignature) {
                    ...
                    if (wasShorthand && this.tok.tokenId != TokenID.LCurly) {
                        ...
                        //this.chkCurTok(TokenID.THEN, "Expected 'then'", errorRecoverySet |
                        //         ErrorRecoverySet.StmtStart);
                        var svInFnc = this.inFnc;
                        isAnonLambda = wasShorthand;
                        this.inFnc = true;
                        this.parseStmtList(errorRecoverySet | ErrorRecoverySet.End |
                                      ErrorRecoverySet.StmtStart,
                                      bod, true, BlockEndElements.None, allowedElements, parentModifiers);
                                                ...
                                                expectClosingRParen: bool): bool 
            {
                ...
                while (true) {
                    ...
                    if (this.inferPropertiesFromThisAssignment && this.tok.tokenId == TokenID.SELF) {
                        if (!isClassConstr) {
            ...
            public parseFncDecl(  errorRecoverySet: ErrorRecoverySet,
                ...
                var prefixNames: ASTList = null;
                var prefixToks: Token[]=[];
                this.nestingLevel = 0;
                ...
                if (!isMethod && !isStatic && !indexer && !lambdaArgContext) {
                    ...
                    if ((this.tok.tokenId != TokenID.ID) && (!convertTokToID(this.tok, this.strictMode))) {
                        ...
                        var prefixName = Identifier.fromToken(this.tok);
                        prefixName.minChar = this.scanner.startPos;
                        prefixName.limChar = this.scanner.pos;
                        ...
                        while (this.tok.tokenId == TokenID.Dot || this.tok.tokenId == TokenID.Colon) {
                            if (prefixNames == null)
                                prefixNames = new ASTList();
                            prefixToks[prefixToks.length] = this.tok;
                            prefixNames.append(prefixName);
                            this.tok = this.scanner.scan();
                            if ((this.tok.tokenId != TokenID.ID) && (!convertTokToID(this.tok, this.strictMode))) {
                                if (isDecl) {
                                    this.reportParseError("Function declaration must include identifier");
                                ...
                                if (prefixNames == null)
                                    prefixNames = new ASTList();
                                prefixName = Identifier.fromToken(this.tok);
                                prefixName.minChar = this.scanner.startPos;
                                prefixName.limChar = this.scanner.pos;
                                this.tok = this.scanner.scan();
                            }
                ...
                var funcDecl: FuncDecl =
                    this.parseFunctionStatements(errorRecoverySet | ErrorRecoverySet.End,
                ...
                funcDecl.prefixNames = prefixNames;
                funcDecl.prefixToks = prefixToks;
            ...
            public parseBaseList(extendsList: ASTList, implementsList: ASTList, errorRecoverySet: ErrorRecoverySet, interfaceOnly: bool, isClass: bool): void {
                ...
                if ((this.tok.tokenId == TokenID.ID) || convertTokToID(this.tok, this.strictMode)) {
                    ...
                    baseName = <Identifier>this.parseNamedType(errorRecoverySet | ErrorRecoverySet.StmtStart,
                ...
                // in lua interface not function body
                //if (this.tok.tokenId == TokenID.LParen) {
                //    if (isClass) {
        ...
        public parseClassElements(classDecl: ClassDecl, errorRecoverySet: ErrorRecoverySet, parentModifiers: Modifiers) {
            ...
            while (!(this.tok.tokenId == TokenID.END || this.tok.tokenId == TokenID.EOF)) {
                var scanNext = true;
            ...
            if (this.tok.tokenId == TokenID.END) {
        ...
        public parseClassMethodAbridgeFunctionDecl() {
        ...
        public parseInterfaceDecl(errorRecoverySet: ErrorRecoverySet, modifiers: Modifiers): TypeDecl {
            ...
            var prevInInterfaceDecl = this.inInterfaceDecl;
            this.inInterfaceDecl = true;
            this.parseInterfaceMembers(errorRecoverySet | ErrorRecoverySet.StmtStart, members);
            this.inInterfaceDecl = prevInInterfaceDecl;
            this.chkCurTok(TokenID.END, "Expected 'end'", errorRecoverySet);
        ...
        public parsePropertyDecl(errorRecoverySet: ErrorRecoverySet, modifiers: Modifiers, requireSignature: bool, isStatic: bool): AST {
            ...
            //else if (this.tok.tokenId == TokenID.ELSEIF) {
            //    if (requireSignature) {
            //        this.tok = this.scanner.scan();
            //        if (this.tok.tokenId == TokenID.LParen) {
            //            isNew = true;
        ...
        public parseVarDecl(errorRecoverySet: ErrorRecoverySet,
            ...
            for (var i=0 ; ;i++) {//TODO:add index
                ...
                if (this.strictMode && (text == "eval")) {//TODO:add code for lua different
            ...
            if (this.tok.tokenId == TokenID.Asg) {
                // TODO: note assignment for language service
                this.tok = this.scanner.scan();
                for (var i = 0 ;i< varDeclList.length ; i++) {//TODO:add index
                    varDecl = varDeclList[i];
                    varDecl.init = this.parseExpr(ErrorRecoverySet.Comma | errorRecoverySet,
        ...
        public parseMemberList(errorRecoverySet: ErrorRecoverySet): ASTList {
            ...
            for (; ;) {
                ...
                else if (this.tok.tokenId == TokenID.Asg) {
                    this.tok = this.scanner.scan();
        ...
        public parseTerm(errorRecoverySet: ErrorRecoverySet, allowCall: bool, typeContext: TypeContext, inCast: bool): AST {
            ...
            if (ast == null) {
                switch (this.tok.tokenId) {
                    ...
                    case TokenID.LCurly://TODO:add table parse
                        minChar = this.scanner.startPos;
                        this.tok = this.scanner.scan();
                ...
                if (ast.nodeType == NodeType.Name) {
                    ...
                    if (this.inferPropertiesFromThisAssignment && 
                        (this.tok.tokenId == TokenID.Colon || this.tok.tokenId == TokenID.Asg) &&
                         ...
                         (<BinaryExpression>ast).operand1.nodeType == NodeType.Self) {
            ...
            for (; ;) {
                switch (this.tok.tokenId) {
                    ...
                    case TokenID.Colon: {
                        ...
                        ast = new BinaryExpression(tokenId==TokenID.Dot?NodeType.Dot:NodeType.Colon, ast, name);
                        ast.minChar = lhsMinChar;
                        ast.limChar = this.scanner.lastTokenLimChar();
                        break;
                    }
        ...
        public parseBlock(errorRecoverySet: ErrorRecoverySet,labelList: ASTList,blockEndElements:BlockEndElements,parentModifiers: Modifiers): AST {
            var modifiers = parentModifiers;
            var minChar = this.scanner.startPos;
            if (hasFlag(modifiers, Modifiers.Exported)) {
            ...
            this.parseStmtList(errorRecoverySet | ErrorRecoverySet.End, block.stmts, false, blockEndElements,
                ...
                switch (this.tok.tokenId) {
                    ...
                    case TokenID.PRIVATE:
                        ...
                        if (this.parsingClassConstructorDefinition) {
                            ...
                            minChar = this.scanner.pos;
                            if (this.inferPropertiesFromThisAssignment && (this.tok.tokenId != TokenID.SELF || (this.tok = this.scanner.scan()).tokenId != TokenID.Dot)) {
                    ...
                    case TokenID.LOCAL:
                        ...
                        if (this.tok.tokenId == TokenID.FUNCTION) {
                            minChar = this.scanner.pos;
                            ast = this.parseFncDecl(errorRecoverySet, false, false, false, null, false, false, false, Modifiers.None, null, true);
                            (<FuncDecl>ast).fncFlags |= FncFlags.LocalFunction;
                            ast.minChar = minChar;
                            limChar = this.scanner.lastTokenLimChar();
                            ast.limChar = limChar;
                        } else {
                            ...
                            if (declAst.nodeType == NodeType.VarDecl) {
                                ...
                                var aList = <ASTList>declAst;
                                aList.members[0].minChar = minChar;
                                ast = new Block(aList, false);
                            }
                            needTerminator = true;
                        }
                    ...
                    case TokenID.FOR:
                        ...
                        //          ErrorRecoverySet.ExprStart | ErrorRecoverySet.Local);
                        ...
                        if (this.tok.tokenId == TokenID.IN) {
                            ...
                            this.chkCurTok(TokenID.DO, "Expected 'do'",
                                      errorRecoverySet | ErrorRecoverySet.End);
                            this.pushStmt(forStmt, labelList);
                            forStmt.body = this.parseBlock(errorRecoverySet, labelList,BlockEndElements.None,  parentModifiers);
                            this.chkCurTok(TokenID.END, "Expected 'end'",
                                      errorRecoverySet | ErrorRecoverySet.None);
                            this.popStmt();
                    ...
                    case TokenID.WHILE: {
                        ...
                        //this.chkCurTok(TokenID.RParen, "Expected ')'", errorRecoverySet |
                        //          ErrorRecoverySet.StmtStart);
                        this.chkCurTok(TokenID.DO, "Expected 'do'", errorRecoverySet |
                                  ErrorRecoverySet.StmtStart);
                        this.pushStmt(whileStmt, labelList);
                        whileStmt.body = this.parseBlock(errorRecoverySet, labelList,BlockEndElements.None,  parentModifiers);
                    ...
                    case TokenID.REPEAT: {
                        mayNotBeExported();
                        if (modifiers != Modifiers.None) {
                            this.reportParseError("'repeat' statement does not take modifiers");
                        }
                        minChar = this.scanner.startPos;
                        this.tok = this.scanner.scan();
                        var doStmt = new RepeatStatement();
                        doStmt.minChar = minChar;
                        this.pushStmt(doStmt, labelList);
                        doStmt.body = this.parseBlock(errorRecoverySet | ErrorRecoverySet.While,
                                                   labelList, BlockEndElements.RepeatBlock,parentModifiers);
                        this.popStmt();
                        doStmt.untilAST = new Identifier("until");
                        doStmt.untilAST.minChar = this.scanner.startPos;
                        this.chkCurTok(TokenID.UNTIL, "Expected 'until'", errorRecoverySet |
                                  ErrorRecoverySet.LParen);
                        doStmt.untilAST.limChar = doStmt.untilAST.minChar + 5;
                        //this.chkCurTok(TokenID.LParen, "Expected '('", errorRecoverySet |
                        //          ErrorRecoverySet.ExprStart);
                    ...
                    case TokenID.IF: {
                        ...
                        ifStmt.thenBod = this.parseBlock(ErrorRecoverySet.Else | errorRecoverySet, labelList, BlockEndElements.IfBlock, parentModifiers);
                        ifStmt.limChar = ifStmt.thenBod.limChar;

                        while (this.tok.tokenId == TokenID.ELSEIF) {
                            this.tok = this.scanner.scan();
                            var elseifStmt = new IfStatement(this.parseExpr(errorRecoverySet |
                                                             ErrorRecoverySet.Then,
                                                             OperatorPrecedence.No, true,
                                                             TypeContext.NoTypes));
                            this.chkCurTok(TokenID.THEN, "Expected 'then'", errorRecoverySet |ErrorRecoverySet.StmtStart);
                            elseifStmt.thenBod = this.parseBlock(ErrorRecoverySet.Else | errorRecoverySet, labelList, BlockEndElements.IfBlock, parentModifiers);
                            elseifStmt.minChar = minChar;
                            ifStmt.addElseif(elseifStmt);
                        }

                       if (this.tok.tokenId == TokenID.ELSE) {
                            this.tok = this.scanner.scan();
                            ifStmt.elseBod = this.parseBlock(errorRecoverySet, labelList,BlockEndElements.None,  parentModifiers);
                    ...
                    //    break;
                    //}
                    case TokenID.LCurly: {//TODO:block lua different need update
                        this.tok = this.scanner.scan();
                        var block = this.parseBlock(errorRecoverySet, labelList,BlockEndElements.None, modifiers);
                        this.chkCurTok(TokenID.RCurly, "Expected '}'", errorRecoverySet);
                        ast = block;
                        break;
                    }
                    case TokenID.SColon:
                        ...
                        break;
                    case TokenID.BREAK:
                    case TokenID.UNTIL: {//TODO:until error
                        mayNotBeExported();
                        if (modifiers != Modifiers.None) {
        ...
        public parseStmtList(errorRecoverySet: ErrorRecoverySet, stmts: ASTList, sourceElms: bool, blockEndElements: BlockEndElements,
            ...
            var ifEndElement = (blockEndElements & BlockEndElements.IfBlock) != BlockEndElements.None;
            var repeatElement = (blockEndElements & BlockEndElements.RepeatBlock) != BlockEndElements.None;
            ...
            this.nestingLevel++;
            for (; ;) {
                if ((this.tok.tokenId == TokenID.RCurly) || (this.tok.tokenId == TokenID.END)||
                    (ifEndElement&&((this.tok.tokenId == TokenID.ELSE) || (this.tok.tokenId == TokenID.ELSEIF)))||
                    (repeatElement && (this.tok.tokenId == TokenID.UNTIL)) ||
        ...
        public parse(sourceText: ISourceText, filename: string, unitIndex: number, allowedElements = AllowedElements.Global): Script {
            ...
            this.parseStmtList(ErrorRecoverySet.EOF | ErrorRecoverySet.Func,
                            bod, true, BlockEndElements.None, allowedElements, Modifiers.None);
