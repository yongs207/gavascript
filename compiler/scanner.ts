    ...
    */
    ...
    export class Scanner implements IScanner {
        ...
        public finishMultilineComment(): bool {
            var ch2: number,ch3:number,ch4:number;
            this.lexState = LexState.InMultilineComment;
            while (this.pos < this.len) {
                if (this.ch == LexCodeMIN) {
                    ch2 = this.peekCharAt(this.pos + 1);
                    ch3 = this.peekCharAt(this.pos + 2);
                    ch4 = this.peekCharAt(this.pos + 3);
                    if (ch2 == LexCodeMIN && ch3 == LexCodeRBR && ch4 == LexCodeRBR) {
                        this.advanceChar(4);
                        if (this.mode == LexMode.File) {
                            this.tokenStart();
                        }
        ...
        public innerScan(): Token {
            ...
            start: while (this.pos < this.len) {
                ...
                else if (this.ch == LexCodeMIN) {
                    if (this.peekCharAt(this.pos + 1) == LexCodeMIN) {
                        var commentText;
                        if (this.peekCharAt(this.pos + 2) == LexCodeLBR && this.peekCharAt(this.pos + 3) == LexCodeLBR) {
                            if (!this.interveningWhitespace) {
                                this.interveningWhitespacePos = this.pos - 1;
                            }
                            var commentStartPos = this.pos - 1;
                            var commentStartLine = this.line;
                            this.advanceChar(4);  // Skip the "--[["
                            this.finishMultilineComment();
